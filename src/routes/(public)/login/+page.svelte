<script>
	...
	import { login } from '$lib/database/auth.js';
	...
	async function handleLogin() {
		...
		if (result.success) {
			userSession.login(result.user); // Guardamos en el store (y localStorage)
			goto('/inicio'); // Redirigimos al CRUD (que ahora está en la raíz protegida)
		} else {
...
<main class="min-h-screen flex items-center justify-center bg-base-200">
	...
	<div class="card w-full max-w-md bg-base-100 shadow-xl">
		<div class="card-body">
			...
			<h1 class="card-title text-3xl font-bold justify-center mb-4">Iniciar Sesión</h1>
